...
class MNIST_CONV_ENCODE(torch.nn.Module):
    def __init__(self):
        super(MNIST_CONV_ENCODE, self).__init__()

        self.conv1  = sbl.BoundEncodeConv2d(1, 64, stride=2)
        self.mem_c1 = sbl.BoundMemUpdate()
...
class MNIST_FC_ENCODE(torch.nn.Module):
    def __init__(self):
        super(MNIST_FC_ENCODE, self).__init__()

        self.flat   = sbl.BoundFlatten()

        self.fc1  = sbl.BoundEncodeLinear(784, 512)
        self.mem_f1 = sbl.BoundMemUpdate()
...
class NMNIST_CONV(torch.nn.Module):
    def __init__(self):
        super(NMNIST_CONV, self).__init__()

        self.conv1  = sbl.BoundConv2d(2, 64, stride=2)
        self.mem_c1 = sbl.BoundMemUpdate()
        ...
        self.fc1    = sbl.BoundLinear(9 * 9 * 128, 256)
        self.mem_f1 = sbl.BoundMemUpdate()
    ...
    def forward(self, x_):
        x = x_.permute(1, 0, 2, 3, 4)[start_T : start_T + time_step, :, :, :, :].float()
    ...
    def snn_bound(self, x_, eps, labels): 
        ...
        s_U = x_.permute(1, 0, 2, 3, 4)[start_T : start_T + self.time_step_certify, :, :, :, :]
        s_L = s_U.clone()
        idx = (torch.zeros(s_U.size()) + eps) > torch.rand(s_U.size()) 
...
class NMNIST_FC(torch.nn.Module):
    def __init__(self):
        super(NMNIST_FC, self).__init__()

        self.flat   = sbl.BoundFlatten()

        self.fc1    = sbl.BoundLinear(2*34*34, 512)
        self.mem_f1 = sbl.BoundMemUpdate()
