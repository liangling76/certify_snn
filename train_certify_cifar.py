...
checkpoint_org = './ckpt/model/cifar_' + model + '.pth.tar'

checkpoint_new = './ckpt/model_cifar/' + model + '_T_' + time_step_certify + '_eps_' + max_eps +  '.pth.tar'
checkpoint_ibp = './ckpt/model_cifar/' + model + '_T_' + time_step_certify + '_eps_' + max_eps +  '_ibp.pth.tar'
checkpoint_crown = './ckpt/model_cifar/' + model + '_T_' + time_step_certify + '_eps_' + max_eps +  '_crown.pth.tar'
checkpoint_analyze = './ckpt/model_cifar/analyze_' + model + '_T_' + time_step_certify + '_eps_' + max_eps +  '.pth.tar'


transform_train = transforms.Compose([transforms.RandomCrop(32, padding=4, padding_mode='edge'), transforms.RandomHorizontalFlip(), transforms.ToTensor()])
# transform_train = transforms.Compose([transforms.RandomCrop(32, padding=4, padding_mode='edge'), transforms.RandomHorizontalFlip(), transforms.ToTensor(), transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
train_dataset   = torchvision.datasets.CIFAR10(root= '/home/lingliang/data', train=True, download=True, transform=transform_train)
...
test_dataset   = torchvision.datasets.CIFAR10(root= '/home/lingliang/data', train=False, download=True, transform=transform_test)
...
if model == 'CONV1':
    snn = CIFAR_CONV1().cuda()
elif model == 'CONV2':
    snn = CIFAR_CONV2().cuda()
...
for epoch in range(num_epochs + finetune_epochs + 1):
    ...
    if if_train and epoch > 0:
        ...
        for i, (images, labels) in enumerate(train_loader):
            ...
            # loss2.backward()
            # (loss1 * 0.8 + loss2 * 0.2).backward()
            if i % 2 == 0:
                loss1.backward()
            else:
                (loss1 * 0.6 + loss2 * 0.4).backward()
    ...
    if if_train:
        ...
        if loss_crown < best_crown:
            loss_crown = best_crown
            torch.save(snn.state_dict(), checkpoint_crown)
        
        if loss_ibp < best_ibp:
            loss_ibp = best_ibp
